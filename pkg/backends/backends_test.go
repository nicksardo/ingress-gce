...
func TestBackendInstanceGroupClobbering(t *testing.T) {
	...
	// seed wantGroups with the first group native to this controller
	wantGroups := sets.NewString(fmt.Sprintf("zones/%s/instanceGroups/%s", defaultZone, "k8s-ig--uid1"))
	for _, newGroup := range newGroups {
		igPath, err := utils.ResourcePath(newGroup.Group)
		if err != nil {
			t.Fatalf("%v", err)
		}
		wantGroups.Insert(igPath)
	}
	if !wantGroups.Equal(gotGroups) {
		t.Fatalf("GetGlobalBackendService(%v) = %v, want %v", be.Name, gotGroups, wantGroups)
	}
}
...
func TestLinkBackendServiceToNEG(t *testing.T) {
	...
	for _, be := range bs.Backends {
		negResource := "networkEndpointGroups"
		if !strings.Contains(be.Group, negResource) {
			t.Errorf("Got backend link %q, want containing %q", be.Group, negResource)
		}
	}
}
